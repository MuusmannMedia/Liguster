...
export default function ChatScreen() {
  ...
  // Første load: beskeder + titel + avatars
  useEffect(() => {
    if (!threadId) return;
    let cancelled = false;
  ...
  const handleSend = async () => {
    ...
    setMessages((prev) => {
      ...
      return prev.map((m) => (m.id === tempId ? (inserted as any) : m));
    });
  ...
  const handleLongPressDelete = (message: Message) => {
    ...
    Alert.alert("Slet besked?", "Denne handling kan ikke fortrydes.", [
      { text: "Annuller", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          else setMessages((prev) => prev.filter((m) => m.id !== message.id));
        },
      },
    ]);
  };
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: "#7C8996", paddingTop: 42 },
  headerRow: {
  ...
  header: {
    ...
    textTransform: "uppercase", paddingHorizontal: 2, flexWrap: "wrap",
  },
  row: { width: "100%", flexDirection: "row", alignItems: "flex-end", paddingHorizontal: 12, marginBottom: 10 },
  rowLeft: { justifyContent: "flex-start" },
  rowRight: { justifyContent: "flex-end" },
  bubbleWrap: { maxWidth: "76%", flexShrink: 1 },
  bubble: {
    paddingVertical: 12, paddingHorizontal: 16, borderRadius: 18,
  ...
  bubbleText: { fontSize: 15, color: "#222", lineHeight: 20, flexShrink: 1, flexWrap: "wrap" },
  time: { fontSize: 11, color: "#a1a1a1", marginTop: 4 },
  timeLeft: { alignSelf: "flex-start", marginLeft: 6 },
  timeRight: { alignSelf: "flex-end", marginRight: 6 },
  avatar: { width: AVATAR_SIZE, height: AVATAR_SIZE, borderRadius: AVATAR_SIZE / 2, marginHorizontal: 6, backgroundColor: "#ddd" },
  avatarBadge: {
    width: AVATAR_SIZE, height: AVATAR_SIZE, borderRadius: AVATAR_SIZE / 2, marginHorizontal: 6,
    backgroundColor: FALLBACK_COLOR, alignItems: "center", justifyContent: "center",
  },
  ...
  input: {
    flex: 1, backgroundColor: "#fff", borderRadius: 8, paddingHorizontal: 16, fontSize: 16,
    marginRight: 9, minHeight: 44, color: "#1e2330", textAlignVertical: "top", maxHeight: 120,
  },
  sendBtn: { alignItems: "center", justifyContent: "center", minWidth: 50, minHeight: 44 },
  sendBtnText: { color: "#fff", fontWeight: "bold", fontSize: 17, letterSpacing: 1, textTransform: "uppercase" },
});