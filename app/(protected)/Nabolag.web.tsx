...
export default function NabolagWeb() {
  // Backup: sikre scroll/klik globalt (Vercel/SSR)
  useEffect(() => {
  ...
  const isGrid = cols > 1;
  const rawW =
    (boardW - GRID.padX * 2 - (isGrid ? GRID.gap * (cols - 1) : 0)) /
    (isGrid ? cols : 1);
  ...
  return (
    <View style={styles.page}>
      <View style={[styles.board, { width: boardW }]}>
        ...
        <View
          style={[styles.filters, isNarrow && { flexDirection: "column", gap: 10 }]}
        >
          <TextInput
            value={searchQuery}
        ...
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={{ color: THEME.sub, marginTop: 14 }}>
                Ingen opslag fundet.
              </Text>
            }
            refreshControl={
